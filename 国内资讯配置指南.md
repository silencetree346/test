# 国内咖啡资讯获取配置指南 🇨🇳

## 📋 概述

由于浏览器的 **CORS（跨域资源共享）限制**，前端无法直接访问大多数国内新闻网站的API。要获取国内咖啡行业资讯，需要配置后端API代理。

---

## 🎯 推荐的国内资讯源

### 商业财经类

| 平台 | 特点 | 适合场景 |
|------|------|---------|
| **36氪** | 科技创投视角，关注新消费品牌 | 行业趋势、融资动态、创业公司 |
| **虎嗅** | 深度商业分析，行业洞察 | 市场分析、行业报告、趋势判断 |
| **钛媒体** | 科技商业资讯，数据分析 | 数据报告、行业研究、商业模式 |
| **艾瑞咨询** | 专业市场研究报告 | 市场规模、用户画像、趋势预测 |

### 咖啡行业垂直媒体

| 平台 | 特点 | 适合场景 |
|------|------|---------|
| **咖门** | 咖啡行业垂直媒体 | 产品创新、品牌动态、门店运营 |
| **品牌星球** | 新消费品牌观察 | 品牌策略、营销案例、消费趋势 |
| **FBIF食品饮料创新** | 食品饮料行业创新 | 产品研发、配方创新、供应链 |

---

## 🚀 方案1：使用 Node.js 后端代理（推荐）

### 步骤1：创建后端项目

```bash
# 创建项目文件夹
mkdir coffee-news-backend
cd coffee-news-backend

# 初始化 Node.js 项目
npm init -y

# 安装依赖
npm install express axios cors dotenv
```

### 步骤2：创建 API 代理服务

创建 `server.js`：

```javascript
const express = require('express');
const axios = require('axios');
const cors = require('cors');
require('dotenv').config();

const app = express();
const PORT = process.env.PORT || 3001;

// 启用 CORS
app.use(cors());
app.use(express.json());

// 36氪资讯API代理
app.get('/api/36kr-news', async (req, res) => {
    try {
        const { keyword = '咖啡', days = 1 } = req.query;
        
        // 36氪的API（需要申请API Key）
        const response = await axios.get('https://api.36kr.com/api/search', {
            params: {
                q: keyword,
                type: 'article',
                per_page: 20
            },
            headers: {
                'Authorization': `Bearer ${process.env.KR36_API_KEY}`
            }
        });
        
        const articles = response.data.data.items.map(item => ({
            title: item.title,
            url: item.url,
            source: '36氪',
            publish_date: item.published_at.split(' ')[0],
            summary: item.summary || item.title
        }));
        
        res.json(articles);
    } catch (error) {
        console.error('36氪API错误:', error.message);
        res.status(500).json({ error: '获取36氪资讯失败' });
    }
});

// 虎嗅资讯API代理
app.get('/api/huxiu-news', async (req, res) => {
    try {
        const { keyword = '咖啡', days = 1 } = req.query;
        
        // 虎嗅的API或RSS
        const response = await axios.get('https://www.huxiu.com/v2_action/article_list', {
            params: {
                type: 0,
                page: 1,
                pagesize: 20
            }
        });
        
        // 过滤咖啡相关内容
        const articles = response.data.data
            .filter(item => item.title.includes(keyword))
            .map(item => ({
                title: item.title,
                url: `https://www.huxiu.com/article/${item.aid}.html`,
                source: '虎嗅',
                publish_date: new Date(item.published_at * 1000).toISOString().split('T')[0],
                summary: item.description || item.title
            }));
        
        res.json(articles);
    } catch (error) {
        console.error('虎嗅API错误:', error.message);
        res.status(500).json({ error: '获取虎嗅资讯失败' });
    }
});

// 咖门资讯（RSS订阅）
app.get('/api/kafmen-news', async (req, res) => {
    try {
        // 使用 RSS 解析
        const Parser = require('rss-parser');
        const parser = new Parser();
        
        const feed = await parser.parseURL('https://www.kafmen.cn/feed');
        
        const articles = feed.items.slice(0, 20).map(item => ({
            title: item.title,
            url: item.link,
            source: '咖门',
            publish_date: new Date(item.pubDate).toISOString().split('T')[0],
            summary: item.contentSnippet || item.title
        }));
        
        res.json(articles);
    } catch (error) {
        console.error('咖门RSS错误:', error.message);
        res.status(500).json({ error: '获取咖门资讯失败' });
    }
});

// 聚合所有国内资讯
app.get('/api/chinese-coffee-news', async (req, res) => {
    try {
        const { days = 1 } = req.query;
        
        // 并发请求多个资讯源
        const [kr36, huxiu, kafmen] = await Promise.allSettled([
            axios.get(`http://localhost:${PORT}/api/36kr-news?days=${days}`),
            axios.get(`http://localhost:${PORT}/api/huxiu-news?days=${days}`),
            axios.get(`http://localhost:${PORT}/api/kafmen-news?days=${days}`)
        ]);
        
        const allNews = [];
        
        if (kr36.status === 'fulfilled') allNews.push(...kr36.value.data);
        if (huxiu.status === 'fulfilled') allNews.push(...huxiu.value.data);
        if (kafmen.status === 'fulfilled') allNews.push(...kafmen.value.data);
        
        // 按日期排序
        allNews.sort((a, b) => new Date(b.publish_date) - new Date(a.publish_date));
        
        console.log(`✅ 成功聚合 ${allNews.length} 条国内资讯`);
        res.json(allNews);
    } catch (error) {
        console.error('聚合资讯错误:', error.message);
        res.status(500).json({ error: '聚合国内资讯失败' });
    }
});

app.listen(PORT, () => {
    console.log(`🚀 咖啡资讯API服务已启动: http://localhost:${PORT}`);
});
```

### 步骤3：配置环境变量

创建 `.env` 文件：

```env
PORT=3001
KR36_API_KEY=your_36kr_api_key_here
HUXIU_API_KEY=your_huxiu_api_key_here
```

### 步骤4：启动后端服务

```bash
node server.js
```

### 步骤5：修改前端代码

在 `docs/index.html` 的 `fetchChineseNews` 方法中：

```javascript
async fetchChineseNews() {
    console.log('🇨🇳 获取国内咖啡资讯...');
    
    try {
        // 调用本地后端API
        const response = await axios.get('http://localhost:3001/api/chinese-coffee-news', {
            params: { days: 1 },
            timeout: 10000
        });
        
        console.log(`✅ 成功获取 ${response.data.length} 条国内资讯`);
        return response.data;
    } catch (error) {
        console.error('❌ 国内资讯获取失败:', error.message);
        return [];
    }
}
```

---

## 🚀 方案2：使用 Python Flask 后端（适合有Python基础的用户）

### 步骤1：创建 Flask 项目

```bash
# 创建项目文件夹
mkdir coffee-news-backend
cd coffee-news-backend

# 创建虚拟环境
python3 -m venv venv
source venv/bin/activate  # Mac/Linux
# venv\Scripts\activate  # Windows

# 安装依赖
pip install flask flask-cors requests feedparser
```

### 步骤2：创建 `app.py`

```python
from flask import Flask, request, jsonify
from flask_cors import CORS
import requests
import feedparser
from datetime import datetime, timedelta

app = Flask(__name__)
CORS(app)  # 允许跨域

@app.route('/api/chinese-coffee-news', methods=['GET'])
def get_chinese_coffee_news():
    """聚合国内咖啡资讯"""
    days = int(request.args.get('days', 1))
    keyword = request.args.get('keyword', '咖啡')
    
    all_news = []
    
    # 1. 获取36氪资讯
    try:
        kr36_news = fetch_36kr_news(keyword, days)
        all_news.extend(kr36_news)
        print(f"✅ 36氪: {len(kr36_news)}条")
    except Exception as e:
        print(f"❌ 36氪失败: {str(e)}")
    
    # 2. 获取虎嗅资讯
    try:
        huxiu_news = fetch_huxiu_news(keyword, days)
        all_news.extend(huxiu_news)
        print(f"✅ 虎嗅: {len(huxiu_news)}条")
    except Exception as e:
        print(f"❌ 虎嗅失败: {str(e)}")
    
    # 3. 获取咖门RSS
    try:
        kafmen_news = fetch_kafmen_rss()
        all_news.extend(kafmen_news)
        print(f"✅ 咖门: {len(kafmen_news)}条")
    except Exception as e:
        print(f"❌ 咖门失败: {str(e)}")
    
    # 按日期排序
    all_news.sort(key=lambda x: x['publish_date'], reverse=True)
    
    return jsonify(all_news)

def fetch_36kr_news(keyword, days):
    """获取36氪资讯"""
    # 这里需要根据36氪实际API调整
    url = 'https://36kr.com/api/search-column'
    params = {'keyword': keyword, 'per_page': 20}
    
    response = requests.get(url, params=params, timeout=10)
    data = response.json()
    
    articles = []
    for item in data.get('data', {}).get('items', []):
        articles.append({
            'title': item['title'],
            'url': item['item_url'],
            'source': '36氪',
            'publish_date': item['published_at'].split(' ')[0],
            'summary': item.get('summary', item['title'])
        })
    
    return articles

def fetch_huxiu_news(keyword, days):
    """获取虎嗅资讯"""
    # 这里需要根据虎嗅实际API调整
    url = 'https://www.huxiu.com/v2_action/article_list'
    params = {'type': 0, 'page': 1, 'pagesize': 20}
    
    response = requests.get(url, params=params, timeout=10)
    data = response.json()
    
    articles = []
    for item in data.get('data', []):
        if keyword in item['title']:
            articles.append({
                'title': item['title'],
                'url': f"https://www.huxiu.com/article/{item['aid']}.html",
                'source': '虎嗅',
                'publish_date': datetime.fromtimestamp(item['published_at']).strftime('%Y-%m-%d'),
                'summary': item.get('description', item['title'])
            })
    
    return articles

def fetch_kafmen_rss():
    """获取咖门RSS"""
    feed = feedparser.parse('https://www.kafmen.cn/feed')
    
    articles = []
    for entry in feed.entries[:20]:
        articles.append({
            'title': entry.title,
            'url': entry.link,
            'source': '咖门',
            'publish_date': datetime(*entry.published_parsed[:3]).strftime('%Y-%m-%d'),
            'summary': entry.get('summary', entry.title)
        })
    
    return articles

if __name__ == '__main__':
    print('🚀 咖啡资讯API服务已启动: http://localhost:3001')
    app.run(host='0.0.0.0', port=3001, debug=True)
```

### 步骤3：运行后端

```bash
python app.py
```

---

## 🚀 方案3：使用 RSS 订阅源（最简单，但资讯有限）

如果不想配置后端，可以直接在前端使用 RSS 解析服务。

### 使用 RSS2JSON 服务

```javascript
async fetchChineseNews() {
    console.log('🇨🇳 通过RSS获取国内资讯...');
    
    const chineseNews = [];
    
    // 咖门 RSS
    try {
        const rss2jsonUrl = 'https://api.rss2json.com/v1/api.json';
        const response = await axios.get(rss2jsonUrl, {
            params: {
                rss_url: 'https://www.kafmen.cn/feed',
                api_key: 'YOUR_RSS2JSON_API_KEY',  // 需要注册获取
                count: 20
            }
        });
        
        if (response.data.status === 'ok') {
            const articles = response.data.items.map(item => ({
                title: item.title,
                url: item.link,
                source: '咖门',
                publish_date: item.pubDate.split(' ')[0],
                summary: item.description.replace(/<[^>]*>/g, '').substring(0, 150)
            }));
            
            chineseNews.push(...articles);
            console.log(`✅ 咖门: ${articles.length}条`);
        }
    } catch (error) {
        console.warn('⚠️ 咖门RSS获取失败:', error.message);
    }
    
    return chineseNews;
}
```

---

## 📊 API对比

| 方案 | 优点 | 缺点 | 适用场景 |
|------|------|------|---------|
| **Node.js 后端** | 灵活、功能强大、支持多种API | 需要部署服务器 | 生产环境、完整项目 |
| **Python Flask** | 易于开发、爬虫友好 | 需要Python环境 | 快速原型、数据分析 |
| **RSS订阅** | 简单、无需后端 | 资讯源有限、更新慢 | 个人使用、演示 |

---

## 🔧 调试技巧

### 1. 检查CORS问题

在浏览器控制台查看：

```
Access to XMLHttpRequest at 'xxx' from origin 'xxx' has been blocked by CORS policy
```

**解决方法：**
- 后端添加 CORS 头
- 使用代理服务器

### 2. 测试后端API

```bash
# 测试聚合API
curl http://localhost:3001/api/chinese-coffee-news?days=1

# 测试单个资讯源
curl http://localhost:3001/api/36kr-news?keyword=咖啡
```

### 3. 查看网络请求

在浏览器开发者工具 → Network 标签中：
- 检查请求状态（200/404/500）
- 查看响应数据格式
- 检查请求耗时

---

## 🚀 部署到生产环境

### 使用 Vercel 部署（推荐）

1. **创建 `vercel.json`：**

```json
{
  "version": 2,
  "builds": [
    {
      "src": "server.js",
      "use": "@vercel/node"
    }
  ],
  "routes": [
    {
      "src": "/api/(.*)",
      "dest": "server.js"
    }
  ]
}
```

2. **部署：**

```bash
npm install -g vercel
vercel
```

### 使用 Heroku 部署

```bash
# 创建 Procfile
echo "web: node server.js" > Procfile

# 部署
git init
heroku create coffee-news-api
git add .
git commit -m "Initial commit"
git push heroku main
```

---

## 📝 注意事项

### 法律合规

1. **遵守网站的 robots.txt 规则**
2. **不要频繁请求，避免给服务器造成压力**
3. **尊重版权，注明资讯来源**
4. **仅用于个人学习和研究**

### API限制

- 大多数API有请求频率限制
- 需要定期更新API Key
- 注意缓存策略，避免重复请求

---

## 🎯 推荐配置

### 开发环境

```
前端: http://localhost:8000 (docs/index.html)
后端: http://localhost:3001 (Node.js/Python)
```

### 生产环境

```
前端: https://yourusername.github.io/test/
后端: https://coffee-news-api.vercel.app
```

---

## ✅ 完整流程总结

1. **选择方案**：根据技术栈选择 Node.js、Python 或 RSS
2. **配置后端**：创建API代理服务
3. **测试API**：确保后端能正常返回数据
4. **修改前端**：更新 `fetchChineseNews` 方法
5. **本地测试**：验证国内资讯能正常显示
6. **部署上线**：部署后端和前端

---

**需要帮助？** 请查看：
- Node.js 官方文档：https://nodejs.org/
- Flask 官方文档：https://flask.palletsprojects.com/
- RSS2JSON API：https://rss2json.com/

**现在您可以获取丰富的国内咖啡行业资讯了！** 🇨🇳☕✨

